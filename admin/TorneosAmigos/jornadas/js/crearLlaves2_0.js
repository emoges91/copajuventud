...
function crear_fases() {
    ...
    if (BCrearFases.addEventListener) {
        ...
        BCrearFases.addEventListener('click', function(event) {
            crear_lista();
        }, false);
    }
    else if (txtBoton.attachEvent) {
        //IE SUCKS….
        BCrearFases.attachEvent('onclick', function(event) {
            crear_lista();
        });
    }
...
function obtenerElementos() {
    for (var i = 1; i <= array_txt_fases[1]; i++) {
        ...
        var cadena = Sortable.serialize("idpartido_" + i + "_1");
        cadena = cadena.split('&');
        cadena = cadena.join();
        cadena = cadena.split("idpartido_" + i + "_1[]=");
        cadena = cadena.join();
        cadena = cadena.split(',,');
        cadena = cadena.join();
