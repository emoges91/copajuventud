...
$nTotalJornadasGrupos = $oJornadas->getMaxJornadasByEvento($aEventoGrupo['ID']);

//algoritmos para evaluar si los datos enviados estan correctos
$flag = 0;
$flag_chekeados = 0;
for ($i = 1; $i <= $nTotalPartidos; $i++) {
    ...
    $consulta_equipos = mysql_query($str_consulta_equipos, $conn);
...
if (($flag == 0) && ($flag_chekeados == 1)) {
    ...
    for ($i = 1; $i <= $nTotalPartidos; $i++) {
        ...
        if ($_POST[$check_partidos] == 'on') {
            ...
            if (($fila['ID_EQUI_CAS'] != '0') && ($fila['ID_EQUI_VIS'] != '0')) {
                ...
                $consulta_equi_1 = mysql_query($str_consulta_equi_1, $conn);
                $consulta_equi_1 = mysql_query($str_consulta_equi_2, $conn);
            }
    ...
    if (($nTotalJornadasGrupos + 1) == $jornada_actual) {
        $oJornadas->setJornadasToJugadasByEvento($aEventoGrupo['ID']);
    }
    ...
    echo "<script type=\"text/javascript\">
		alert('Marcadores actualizados correctamente');
